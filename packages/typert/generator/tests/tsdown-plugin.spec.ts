...
describe('typertPlugin', () => {
  ...
  it('emits only the release-build batch contributors without workspace discovery', async () => {
    const root = await workspace()
    const trigger = await packageOutput(root, 'generator', { name: '@deepseek-ai/dsh-typert-generator' })
    await packageOutput(root, 'core/tools', {
      name: '@deepseek-ai/dsh-tools',
    ...
    typertPlugin({ mode: 'workspace', faces: ['host'], packages: ['@deepseek-ai/dsh-tools'] })
      .writeBundle({ dir: trigger })
