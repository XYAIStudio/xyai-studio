...
XYAI Core connect layer: a `SessionFacade` over `AgentRuntime`, text-inferred `TurnCapability`, `PermissionMode` mapping, model catalog + gateway, and a stall watchdog.
...
`PermissionMode` is approval policy only. It never forces a chitchat turn onto tools. Anthropic Messages cannot drive the Codex tools path yet.
