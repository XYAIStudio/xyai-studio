...
| S3 | P0 | 「全盘搜索完成，未发现本地模型」when Ollama was installed but stopped; scan was only `/api/tags` | **Fixed** — auto-start + `ollama list` + disk manifests fallback; dialog distinguishes stopped vs empty |
...
| S5 | P0 | `backend/server.ts` imports `./openxyos-identity` but some trees omit the file → `MODULE_NOT_FOUND` | **Fixed** — Studio writes the upstream-compatible stub when the import exists and the file is missing |
...
| S8 | P1 | Stale picker ref (e.g. `qwen3:8b` not in tags) still attempted chat | **Fixed** — after Ollama is up, missing model name gets a refresh/pull message |
...
2. Stop Ollama → 模型 page shows 「启动 Ollama」; after start, tags list refreshes.
3. Chat `ollama:qwen3:8b` with API down → start action, not `fetch failed`.
4. 业务空间 timeout → error card includes child log / identity hint, not only “55s 未就绪”.
