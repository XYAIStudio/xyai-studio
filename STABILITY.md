...
| S19 | P1 | Chat picker labeled `本地 · raw-tag` and repeated `ollama:tag`; mis-tagged aliases (same digest) looked like different families | **Fixed** — `ollama show` architecture (qwen25vl → Qwen2.5-VL); shared digest → one identity + 别名; never title a qwen blob as DeepSeek |
