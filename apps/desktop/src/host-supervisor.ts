...
const DEFAULT_READINESS_PREFIX = 'dsh web: '
...
function parseReadinessLine(line: string, prefix: string): string | undefined {
  if (!line.startsWith(prefix)) return undefined
  const token = line.slice(prefix.length).split(/\s/u, 1)[0]
  if (token === undefined) throw new Error(`readiness line has no URL: ${line}`)

  let url: URL
  try {
    url = new URL(token)
  } catch {
    throw new Error(`readiness URL is invalid: ${token}`)
  }
  ...
  if (url.protocol !== 'http:'
    ...
    || port < 1
    || port > 65_535) {
    throw new Error(`readiness URL must be loopback HTTP with an explicit port: ${token}`)
  }
  // Current DSH web profiles publish a loopback URL with a one-time local
  // authentication token. Validate through URL, but keep the original token
  // text so legacy URLs also retain their existing slash-less form.
  return token
}
 ...
 */
export function createReadinessParser(prefix = DEFAULT_READINESS_PREFIX): ReadinessParser {
  let pending = ''
  let readyUrl: string | undefined

  const accept = (line: string): string | undefined => {
    const parsed = parseReadinessLine(line.replace(/\r$/u, ''), prefix)
    if (parsed === undefined) return undefined
    if (readyUrl !== undefined && parsed !== readyUrl) {
      throw new Error(`conflicting readiness URLs: ${readyUrl} and ${parsed}`)
    }
    readyUrl = parsed
    return readyUrl
  }
...
export interface HostSupervisorOptions {
  ...
  /** Readiness line prefix; defaults to `dsh web: ` (XYOS backend uses `xyos backend: `). */
  readonly readinessPrefix?: string
...
export function createHostSupervisor(options: HostSupervisorOptions): HostSupervisor {
  ...
  const start = (): Promise<string> => {
    ...
    startPromise = new Promise<string>((resolve, reject) => {
      const parser = createReadinessParser(options.readinessPrefix)
...
export function spawnDshWeb(options: SpawnDshWebOptions): HostChild {
  const env = options.electronRunAsNode
    ...
    : options.env
  const process = spawn(options.nodeExecutable, ['--expose-internals', options.cliEntry, 'web', '--host', '127.0.0.1', '--port', '0', '--no-open'], {
    cwd: options.cwd,
    env,
