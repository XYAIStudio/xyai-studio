...
// The built-in desktop bundle list: the official base and web-app bundles
// followed by the XYAI base product bundle, which ships inside the seed and
// every active profile. Installed plugins may only appear after these entries
// (see profilePluginNames), so product bundles appended here are not
// user-removable.
const DESKTOP_PROFILE_BUNDLES = ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app', '@xyai/dsh-product-base', '@xyai/dsh-product-collab'] as const
