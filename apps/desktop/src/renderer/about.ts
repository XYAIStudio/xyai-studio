/**
 * About overlay — content 照搬 0.3 ABOUT_HTML (XYAI 生态四站).
 * Card links open the in-app Browser zone (not shell.openExternal / iframe nav).
 */

import { openBrowser } from './zones/browser.js';
...
export function showAboutDialog(logoSrc: string): void {
  ...
  overlay.setAttribute('aria-label', '关于我们');
  ...
  close.type = 'button';
  close.className = 'about-frame-close';
  close.textContent = '×';
  ...
  iframe.title = '关于我们';
  iframe.srcdoc = ABOUT_HTML.replaceAll('__LOGO__', logoSrc);
  ...
  const onMsg = (e: MessageEvent) => {
    ...
    if (data && data.type === 'xyai-about-open' && typeof data.url === 'string') {
      openUrl(data.url);
    }
  };
  ...
  iframe.addEventListener('load', () => {
    try {
      ...
      if (!doc) return;
      // Inject click interceptor (srcdoc is same-origin in Chromium)
