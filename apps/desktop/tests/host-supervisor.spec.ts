...
describe('desktop Host readiness', () => {
  ...
  it('preserves the DSH loopback authentication token', () => {
    const parser = createReadinessParser()

    expect(parser.push('dsh web: http://127.0.0.1:4173/?token=desktop-local-token\n'))
      .toBe('http://127.0.0.1:4173/?token=desktop-local-token')
  })
...
describe('desktop Host process', () => {
  it('opts the packaged Electron executable into its Node runtime', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      ['--expose-internals', expect.stringContaining('/Resources/host/node_modules/@deepseek-ai/dsh/lib/bin.js'), 'web', '--host', '127.0.0.1', '--port', '0', '--no-open'],
      expect.objectContaining({ env: { DSH_DESKTOP: '1', ELECTRON_RUN_AS_NODE: '1' } }),
    )
  })

  it('opts the packaged Electron executable into its Node runtime for XYOS', async () => {
    const spawned = {
      stdout: { on: vi.fn(), off: vi.fn() },
      stderr: { on: vi.fn(), off: vi.fn() },
      on: vi.fn(),
    ...
    const { spawnXyosBackend } = await import('../src/xyos-backend.ts')
    spawnXyosBackend({
      nodeExecutable: '/Applications/XYAI Studio.app/Contents/MacOS/XYAI Studio',
      backendDir: '/Applications/XYAI Studio.app/Contents/Resources/xyos-backend',
      env: { PORT: '0' },
      electronRunAsNode: true,
    })

    expect(spawn).toHaveBeenLastCalledWith(
      '/Applications/XYAI Studio.app/Contents/MacOS/XYAI Studio',
      [expect.stringContaining('xyos-backend'), 'server.ts'],
      expect.objectContaining({ env: { PORT: '0', ELECTRON_RUN_AS_NODE: '1' } }),
    )
  })
})
