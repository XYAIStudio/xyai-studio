...
async function main(): Promise<void> {
  ...
  // The XYAI product layer builds through its own package-local commands (the
  // out-of-tree workspace is excluded from the root aggregates), so the bundle
  // and pack steps run here, ahead of the package-set closure walk.
  await runPnpm(['-r', '--filter', '@xyai/*', 'run', 'bundle'], buildEnv, REPOSITORY_ROOT)
  await runPnpm(['run', 'pack:xyai'], buildEnv, APP_ROOT)
